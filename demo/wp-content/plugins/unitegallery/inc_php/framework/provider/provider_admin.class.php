...
defined('_JEXEC') or die('Restricted access');
   ...
   class UniteProviderAdminUG extends UniteGalleryAdmin{
		...
		private function a_________NEW_FEATURES___________(){}
		
		
		/**
		 * add gallery to media upload
		 */
		...
		public function addPostMetaboxes(){
			...
			if(UniteFunctionsWPUG::isInsidePostPage() == false)
				return(false);
			
			add_meta_box("unite_gallery_galleries", __("Unite Gallery - Post Galleries", UNITEGALLERY_TEXTDOMAIN), array(self::$t,"getMetaboxHtml"));
		...
		private function addNewFeatures(){
			...
			if(self::isInsidePlugin() == false){
				self::addAction(self::ACTION_ADD_SCRIPTS, "addWPPostPageScripts");
			}
			
			//add metaboxes
			
			self::addAction(self::ACTION_ADD_METABOXES, 'addPostMetaboxes');
